...
export async function logProductionRun({
  ...
  await prisma.$transaction(async (tx) => {
    // 1. Deduct raw materials used in production (ADJUSTMENT with negative quantityDelta)
    for (const ingredient of ingredients) {
      const stock = await tx.branchStock.findUnique({
        where: {
          branchId_itemId: { branchId, itemId: ingredient.itemId },
        },
      });
    ...
    // 2. Increment finished product yield (PRODUCTION with positive quantityDelta)
    await tx.branchStock.upsert({
      where: {
        branchId_itemId: { branchId, itemId: finishedItemId },
      },
    ...
    await tx.stockTransaction.create({
      data: {
        ...
        branch: { connect: { id: branchId } },
        item: { connect: { id: finishedItemId } },
        user: { connect: { id: activeUserId } },
      },
    });
  });
